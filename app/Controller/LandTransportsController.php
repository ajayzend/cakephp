...
class LandTransportsController extends AppController {
	public $uses = array('LandTransports');
	...
	public function admin_index() {
		...
		$limit = $this->limit;
		$portDetails=$this->LandTransports->find('all');
		$count = count($portDetails);
		$this->paginate = array('limit'=>$limit, 'order'=>array('id'=>'DESC'));  
		$portDetails = $this->Paginator->paginate('LandTransports');
	...
	public function admin_add() {
		if ($this->request -> isPost())
		{
			...
			$this->LandTransports->set($this->data);
			$return = $this->LandTransports->save($this->data);
	...
	public function admin_update($id=null) {
		...
		{
			$this->LandTransports->set($this->data);
			$this->LandTransports->save($this->data);
			return $this->redirect(array('action' => 'index'));
		}
		else{
			$Data = $this->LandTransports->find('first',array('conditions'=>array('id'=>$id)));
	...
	public function admin_delete() {
		$this->autoRender = false;
		if ($this->request -> isPost()) {
		if ($this->LandTransports->delete($this->data['id'], false)) {
	...
	{
		$saleReports=$this->LandTransports->find('all');
