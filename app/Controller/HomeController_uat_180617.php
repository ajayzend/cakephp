...
//unset($Condition);
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			...
			if($userflag == 1) { // Not loggedin
				...
				$this->paginate = array('limit' => 12, 'conditions' => array('Car.publish' => array(0, 1), 'Car.deleted' => 0, 'AND' => array('OR' => $Condition)),
					'order' => 'Car.id DESC', 'joins' => array(
						array(
							'alias' => 'Carmis',
...
//echo '<pre>'; print_r($this->Car->getDataSource()); die;
		...
		if ($this->Session->read('LANGUAGE') == 2) {
			...
			if($userflag == 1) { // Not loggedin
				...
				$MaxRange = $this->CarPayment->find('first', array('fields' => array('MAX(CarPayment.yen) as max_price', 'MIN(CarPayment.yen) as min_price'),
					'conditions' => array('Car.publish' => array(1, 0), 'Car.deleted' => 0, 'AND' => array('OR' => $Condition)), 'joins' => array(
						array(
							'alias' => 'Carmis',
							'table' => 'car_publish_new_sold',
							'type' => 'INNER',
...
function filterStockList()
	{
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			...
			if($userflag == 1) { // Not Loggedin
				...
				$this->paginate = array('limit' => 12, 'fields' => array("CarPaymentAls.*", "CarName.*", 'Car.*'),
					'conditions' => array('Car.publish' => array(1, 0), 'Car.deleted' => 0, $Condition, 'AND' => array('OR' => $GlobalCondition)),
					'page' => $_POST['CurrentPage'], 'order' => $order, 'joins' => array(
						array(
							'alias' => 'Carmis',
							'table' => 'car_publish_new_sold',
	...
	public function globalSearch()
	{
		...
		$this->Car->bindModel(array('belongsTo'=>array('CarName'=>array('fields'=>'car_name,id'))));
		if($userflag == 1) { // User NotLoggedin
			...
			$carNames = $this->Car->find('all', array('conditions' => array('Car.publish' => array(1, 0), 'Car.deleted' => 0,
					'AND' => array('OR' => $conditions)), 'recursive' => 2, 'limit' => 10,
					'joins' => array(
						array(
							'alias' => 'Carmis',
