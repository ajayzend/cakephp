...
class InformationController extends AppController {
	...
	public function admin_index() {
		$limit = $this->limit;
		$portDetails=$this->Information->find('all');
		$count = count($portDetails);
		$this->paginate = array('limit'=>$limit, 'order'=>array('id'=>'DESC'));  
		$portDetails = $this->Paginator->paginate('Information');
	...
	public function admin_add() {
		if ($this->request -> isPost())
		{
			...
			$this->Information->set($this->data);
			$return = $this->Information->save($this->data);
	...
	public function admin_update($id=null) {
		...
		{
			$this->Information->set($this->data);
			$this->Information->save($this->data);
			return $this->redirect(array('action' => 'index'));
		}
		else{
			$Data = $this->Information->find('first',array('conditions'=>array('id'=>$id)));
	...
	public function admin_delete() {
		$this->autoRender = false;
		if ($this->request -> isPost()) {
		if ($this->Information->delete($this->data['id'], false)) {
	...
	{
		$saleReports=$this->Information->find('all');
