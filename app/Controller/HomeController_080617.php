...
//'conditions'=>array('Car.cnumber LIKE'=>'%'.$term.'%')
		...
		if (@$_REQUEST['search']) {
			...
			$Condition[] = array('UPPER(CarName.car_name) LIKE' => '%' . strtoupper($term) . '%');
			$Condition[] = array('UPPER(Car.cnumber) LIKE' => '%' . strtoupper($term) . '%');
			$Condition[] = array('Car.stock LIKE' => '%' . $term . '%');
			$Condition[] = array('UPPER(brandsAls.brand_name) LIKE' => '%' . strtoupper($term) . '%');
			$Condition[] = array('UPPER(car_typesAls.type) LIKE' => '%' . strtoupper($term) . '%');
			$Condition[] = array('UPPER(vehicle_typesAls.type) LIKE' => '%' . strtoupper($term) . '%');
		}
		if (@$_REQUEST['modal']) {
		...
		$cond = array('Car.publish' => 0, 'Car.deleted' => 0, 'CarPaymentAls.created_on <=' => $start, 'CarPaymentAls.created_on >=' => $end);
...
//unset($Condition);
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			$this->paginate = array('limit' => 12, 'conditions' => array('OR' => array(array('Car.publish' => 1, 'Car.deleted' => 0, $Condition),
				array($Condition, $cond))), 'order' => 'Car.id DESC', 'joins' => array(
				array(
					'alias' => 'CarPaymentAls',
					'table' => 'car_payments',
					'type' => 'left',
			...
			$this->paginate = array('limit' => 12,  'conditions' => array('Car.publish' => 1, 'Car.deleted' => 0,'AND' => array('OR'=>$Condition)),
...
//echo '<pre>'; print_r($this->Car->getDataSource()); die;
	...
	//	echo "<pre>";print_r($carDetails);die;
		...
		$KMRange = $this->Car->find('first', array('fields' => array('MAX(Car.mileage) as max_price', 'MIN(Car.mileage) as min_price'),
			'conditions' => array('publish' => 1, 'Car.deleted' => 0, 'AND' => array('OR' => $Condition)) , 'joins' => array(
				array(
					'alias' => 'CarName',
					'table' => 'car_names',
					'type' => 'left',
	...
	function filterStockList()
	{
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			...
			$this->paginate = array('limit' => 12, 'fields' => array("CarPaymentAls.*", "CarName.*", 'Car.*'),
				'conditions' => array('OR' => array(array('Car.publish' => 1, 'Car.deleted' => 0, $Condition), array($Condition, $cond))), 'page' => $_POST['CurrentPage'], 'order' => $order, 'joins' => array(
					array(
						'alias' => 'CarPaymentAls',
						'table' => 'car_payments',
						'type' => 'left',
			...
			$this->paginate = array('limit' => 12, 'fields' => array("CarPaymentAls.*", "CarName.*", 'Car.*'), 'conditions' => array('Car.publish' => 1, 'Car.deleted' => 0, $Condition), 'page' => $_POST['CurrentPage'], 'order' => $order, 'joins' => array(
				array(
					'alias' => 'CarPaymentAls',
					'table' => 'car_payments',
					'type' => 'left',
	...
	public function globalSearch()
	{
		...
		$carNames = $this->Car->find('all' , array('conditions'=>array('Car.publish' => 1, 'Car.deleted' => 0,
			array('UPPER(CarName.car_name) like'=> '%'.strtoupper($_GET['term']).'%')),'recursive'=>2, 'limit' => 10));
