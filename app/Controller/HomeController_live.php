...
class HomeController extends AppController
{
	...
	function allstockList()
	{
		...
		if (@$this->passedArgs['brand'] != "") {
			...
			$carName = $this->CarName->find('list', array('fields' => array('id', 'car_name'), 'conditions' => array('car_name LIKE' => '%' . $term . '%')));
...
*/
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			...
			$this->paginate = array('limit' => 12, 'conditions' => array('OR' => array(array('Car.publish' => 1, $Condition), array($Condition, $cond))), 'order' => 'Car.id DESC', 'joins' => array(
				array(
					'alias' => 'CarPaymentAls',
					'table' => 'car_payments',
					'type' => 'left',
			...
			$this->paginate = array('limit' => 12, 'conditions' => array('Car.publish' => 1, $Condition), 'order' => 'Car.id DESC', 'joins' => array(
				array(
					'alias' => 'CarPaymentAls',
					'table' => 'car_payments',
					'type' => 'left',
		...
		if ($this->Session->read('LANGUAGE') == 2) {
			...
			$this->CarPayment->unbindModelAll();
			$MaxRange = $this->CarPayment->find('first', array('fields' => array('MAX(CarPayment.asking_price) as max_price', 'MIN(CarPayment.asking_price) as min_price'), 'conditions' => array('Car.publish' => 1, $Condition), 'joins' => array(
				array(
					'alias' => 'Car',
					'table' => 'cars',
					'type' => 'left',
			...
			$this->CarPayment->unbindModelAll();
			$MaxRange = $this->CarPayment->find('first', array('fields' => array('MAX(CarPayment.yen) as max_price', 'MIN(CarPayment.yen) as min_price'), 'conditions' => array('Car.publish' => array(1, $userflag), $Condition), 'joins' => array(
				array(
					'alias' => 'Car',
					'table' => 'cars',
					'type' => 'left',
...
/////
	...
	function allstockList_OLD()
	{
		...
		$this->paginate = array('limit' => 12, 'conditions' => array(array('Car.publish' => 1, $Condition)), 'order' => 'Car.id DESC', 'joins' => array(
			array(
				'alias' => 'CarPaymentAls',
				'table' => 'car_payments',
				'type' => 'left',
		...
		if ($this->Session->read('LANGUAGE') == 2) {
			...
			$this->CarPayment->unbindModelAll();
			$MaxRange = $this->CarPayment->find('first', array('fields' => array('MAX(CarPayment.yen) as max_price', 'MIN(CarPayment.yen) as min_price'), 'conditions' => array('Car.publish' => 1, $Condition), 'joins' => array(
				array(
					'alias' => 'Car',
					'table' => 'cars',
					'type' => 'left',
...
/////
	function filterStockList()
	{
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			...
			$this->paginate = array('limit' => 12, 'fields' => array("CarPaymentAls.*", "CarName.*", 'Car.*'), 'conditions' => array('OR' => array(array('Car.publish' => 1, $Condition), array($Condition, $cond))), 'page' => $_POST['CurrentPage'], 'order' => $order, 'joins' => array(
				array(
					'alias' => 'CarPaymentAls',
					'table' => 'car_payments',
					'type' => 'left',
			...
			$this->paginate = array('limit' => 12, 'fields' => array("CarPaymentAls.*", "CarName.*", 'Car.*'), 'conditions' => array('Car.publish' => 1, $Condition), 'page' => $_POST['CurrentPage'], 'order' => $order, 'joins' => array(
				array(
					'alias' => 'CarPaymentAls',
					'table' => 'car_payments',
					'type' => 'left',
	...
	public function chassis_list()
	{
		...
		$this->Car->unbindModelAll();
		$Cars = $this->Car->find('all', array('fields' => array('id', 'cnumber'), 'conditions' => array('Car.cnumber LIKE' => '%' . $term . '%')));
	...
	public function globalSearch()
	{
		...
		foreach ($carNames as $crnm) {
			$CarSearch[] = array("label" => $crnm['CarName']['car_name'] . " " . $crnm['Car']['package'], "vallue" => $crnm['CarName']['car_name'] . " " . $crnm['Car']['package'], "tag" => $crnm['Car']['id']);
		}
