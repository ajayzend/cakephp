...
class RecoveriesController extends AppController {
	...
	public function admin_index() {
		...
		$limit = $this->limit;
		$portDetails=$this->Recovery->find('all');
		$count = count($portDetails);
		$this->paginate = array('limit'=>$limit, 'order'=>array('id'=>'DESC'));  
		$portDetails = $this->Paginator->paginate('Recovery');
	...
	public function admin_add() {
		if ($this->request -> isPost())
		{
			...
			$this->Recovery->set($this->data);
			$return = $this->Recovery->save($this->data);
	...
	public function admin_update($id=null) {
		...
		{
			$this->Recovery->set($this->data);
			$this->Recovery->save($this->data);
			return $this->redirect(array('action' => 'index'));
		}
		else{
			$Data = $this->Recovery->find('first',array('conditions'=>array('id'=>$id)));
	...
	public function admin_delete() {
		$this->autoRender = false;
		if ($this->request -> isPost()) {
		if ($this->Recovery->delete($this->data['id'], false)) {
	...
	{
		$saleReports=$this->Recovery->find('all');
