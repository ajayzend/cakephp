...
//unset($Condition);
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			$this->paginate = array('limit' => 12, 'conditions' => array('AND' => array(array('Car.publish' => array(1, $userflag), 'Car.deleted' => 0, $Condition),
			...
			$this->paginate = array('limit' => 12, 'conditions' => array('Car.publish' => array(1, $userflag), 'Car.deleted' => 0, 'AND' => array('OR' => $Condition), $cond),
...
//echo '<pre>'; print_r($this->Car->getDataSource()); die;
		...
		if ($this->Session->read('LANGUAGE') == 2) {
			...
			$MaxRange = $this->CarPayment->find('first', array('fields' => array('MAX(CarPayment.asking_price) as max_price', 'MIN(CarPayment.asking_price) as min_price'),
				'conditions' => array('Car.publish' => array(1, $userflag), 'Car.deleted' => 0, $Condition), 'joins' => array(
					array(
						'alias' => 'Car',
						'table' => 'cars',
						'type' => 'left',
			...
			$MaxRange = $this->CarPayment->find('first', array('fields' => array('MAX(CarPayment.yen) as max_price', 'MIN(CarPayment.yen) as min_price'),
				'conditions' => array('Car.publish' => array(1, $userflag), 'Car.deleted' => 0, 'AND' => array('OR' => $Condition), $cond), 'joins' => array(
					array(
						'alias' => 'Car',
						'table' => 'cars',
						'type' => 'left',
	...
	function filterStockList()
	{
		...
		if (@$_POST['data']['Home']['stock'] || @$_POST['data']['Home']['cnumber']) {
			...
			$this->paginate = array('limit' => 12, 'fields' => array("CarPaymentAls.*", "CarName.*", 'Car.*'),
				'conditions' => array('Car.publish' => array(1, $userflag), 'Car.deleted' => 0, $Condition, 'AND' => array('OR' => $GlobalCondition), $cond),
	...
	public function chassis_list()
	{
		...
		$Cars = $this->Car->find('all', array('fields' => array('id', 'cnumber'),
			'conditions' => array('Car.publish' => array(1, $userflag), 'Car.deleted' => 0,'Car.cnumber LIKE' => '%' . $term . '%', $cond)
			, 'joins' => array(
					array(
						'alias' => 'CarPaymentAls',
						'table' => 'car_payments',
	...
	public function globalSearch()
	{
		...
		$this->Car->bindModel(array('belongsTo' => array('CarName' => array('fields' => 'car_name,id'))));
		$carNames = $this->Car->find('all',
			array('conditions' => array('Car.publish' => array(1, $userflag), 'Car.deleted' => 0,
				'AND' => array('OR' => $conditions), $cond)
				, 'recursive' => 2,
				'limit' => 10,
				'joins' => array(
					array(
