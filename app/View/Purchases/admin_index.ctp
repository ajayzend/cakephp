...
<div class="box-header well">
						<div class="col-md-12"><h2><i class="fa fa-cogs">&nbsp;</i>Purchases Operations Management</h2></div>
					...
					<div class="row">
						...
						<a href="<?php echo $this->Html->url('/',true);?>admin/purchases/add" class='btn btn-primary btn-lg pull-right hint--bottom'> <i class="fa fa-plus-circle">&nbsp;</i>Add</a>
                        <a href="<?php echo $this->Html->url('/',true);?>admin/purchases/export" class='btn btn-primary btn-lg pull-right hint--bottom'> <i class="fa fa-plus-circle">&nbsp;</i>Export</a>
                        </div>
					</div>
					</div>
        ...
        <input type="hidden" name="modeval">
                        ...
                        <table class="table table-striped table-bordered bootstrap-datatable datatable custom_table">
							  ...
							  ?>
								<tr id="portId_<?php echo $val['Purchase']['id'];?>">
									<td class="center"><?php echo $val['Purchase']['purchase_auction_day']?></td>
									<td class="center"><?php echo $val['Purchase']['purchase_auction_name']?></td>
									<td class="center">
                                    	<a href="<?php echo $this->Html->url('/',true);?>admin/purchases/update/<?php echo $val['Purchase']['id'];?>"><button type="button" class='btn btn-info hint--bottom' ><i class="fa fa-pencil"></i></button></a>
						 
										<a href="javascript:checkDelete(<?php echo $val['Purchase']['id'];?>,<?php echo $pages;?>);" class='btn btn-danger hint--bottom' data-hint="Delete"><i class="fa fa-trash-o"></i></a>
...
<script>
	function delete_port(id,pageNo){
		...
		if(id!= undefined){
			$.ajax({
			   url:'<?php echo $this->Html->url('/admin/purchases/delete',true);?>',
			   type:'POST',
			   data:{'id':id},
			   dataType:'JSON',
			   success:function(data){
				if(data.status='success'){
					window.location = "<?php echo $this->Html->url('/admin/purchases');?>";
				}
			 }
